...
import { collection, query, getDocs, where, addDoc, doc, setDoc, updateDoc } from 'firebase/firestore'
...
const SPSalesReport = () => {
    ...
    const [items, setItems] = useState([
        { name: 'Buns', price: 5.0, starting: 0, delivery: 0, waste: 0, ending: 0, sold: 0, sales: 0 },
        { name: 'HD Buns', price: 5.0, starting: 0, delivery: 0, waste: 0, ending: 0, sold: 0, sales: 0 },
        { name: 'FL Buns', price: 12.0, starting: 0, delivery: 0, waste: 0, ending: 0, sold: 0, sales: 0 },
        { name: 'Patty', price: 15.0, starting: 0, delivery: 0, waste: 0, ending: 0, sold: 0, sales: 0 },
        { name: 'FL Dog', price: 44.0, starting: 0, delivery: 0, waste: 0, ending: 0, sold: 0, sales: 0 },
        { name: 'Hung. Chiz', price: 65.0, starting: 0, delivery: 0, waste: 0, ending: 0, sold: 0, sales: 0 },
        { name: 'Chori Patty', price: 22.5, starting: 0, delivery: 0, waste: 0, ending: 0, sold: 0, sales: 0 },
        { name: 'Cheese', price: 5.0, starting: 0, delivery: 0, waste: 0, ending: 0, sold: 0, sales: 0 },
        { name: 'Bacon', price: 25.0, starting: 0, delivery: 0, waste: 0, ending: 0, sold: 0, sales: 0 },
        { name: 'Cheesy HD', price: 13.5, starting: 0, delivery: 0, waste: 0, ending: 0, sold: 0, sales: 0 },
        { name: 'Egg', price: 15.0, starting: 0, delivery: 0, waste: 0, ending: 0, sold: 0, sales: 0 },
        { name: 'Mineral', price: 16.0, starting: 0, delivery: 0, waste: 0, ending: 0, sold: 0, sales: 0 },
        { name: 'Red Tea', price: 35.0, starting: 0, delivery: 0, waste: 0, ending: 0, sold: 0, sales: 0 },
        { name: 'M. Dew PET', price: 25.0, starting: 0, delivery: 0, waste: 0, ending: 0, sold: 0, sales: 0 },
        { name: 'Pepsi PET', price: 25.0, starting: 0, delivery: 0, waste: 0, ending: 0, sold: 0, sales: 0 },
        { name: '7Up PET', price: 25.0, starting: 0, delivery: 0, waste: 0, ending: 0, sold: 0, sales: 0 },
        { name: 'Miranda PET', price: 25.0, starting: 0, delivery: 0, waste: 0, ending: 0, sold: 0, sales: 0 },
        { name: 'Sting PET', price: 25.0, starting: 0, delivery: 0, waste: 0, ending: 0, sold: 0, sales: 0 },
        { name: 'Tropicana PET', price: 25.0, starting: 0, delivery: 0, waste: 0, ending: 0, sold: 0, sales: 0 },
        { name: 'Rootbeer PET', price: 25.0, starting: 0, delivery: 0, waste: 0, ending: 0, sold: 0, sales: 0 },
        { name: 'Gatorade PET', price: 25.0, starting: 0, delivery: 0, waste: 0, ending: 0, sold: 0, sales: 0 }
    ])
                ...
                try {
                    ...
                    if (!querySnapshot.empty) {
                        ...
                        const shiftQuery = query(
                            shiftsRef,
                            where('name', '==', employeeData.fullName),
                            where('date', '==', today)
                        )
                        ...
                        if (!shiftSnapshot.empty) {
                            ...
                            setSelectedBranch(shiftData.location)
                            const inventoryRef = collection(db, 'inventory')
                            const inventorySnapshot = await getDocs(inventoryRef)

                            if (!inventorySnapshot.empty) {
                                const locationInventory = inventorySnapshot.docs
                                    .map((doc) => doc.data())
                                    .filter(
                                        (item) => item.location?.toLowerCase() === shiftData.location?.toLowerCase()
                                    )

                                if (locationInventory.length > 0) {
                                    const inventoryData = locationInventory.map((item) => ({
                                        name: item.item,
                                    ...
                                    setItems((prevItems) =>
                                        prevItems.map((item) => {
                                            const inventoryItem = inventoryData.find((inv) => inv.name === item.name)
                                            return inventoryItem
                                                ? {
                                                      ...item,
                                                      starting: inventoryItem.quantity,
                                                      ending: inventoryItem.quantity
                                                  }
                                                : item
                                        })
                                    )
                                } else {
                                    console.log(`No inventory items found for location: ${shiftData.location}`)
                                }
                            } else {
                                console.log('No inventory data found.')
                            }
                        } else {
                            console.log("No shifts found for the employee on today's date.")
                            const today = new Date().toLocaleDateString('en-CA')
                            console.log(today)
                        }
                    } else {
    ...
    const TableInput = ({ value, onChange }) => (
        <input type="number" className="border px-2 py-1 w-full" value={value} onChange={onChange} />
    )
        ...
        try {
            ...
            for (const item of items) {
                ...
                if (!inventorySnapshot.empty) {
                    ...
                    const newQuantity = item.ending

                    await updateDoc(inventoryDoc.ref, { quantity: newQuantity })
                }
            }

            alert('Inventory quantities updated successfully.')
        } catch (error) {
                                        ...
                                        <React.Fragment key={day}>
                                            <td className="border p-1">
                                                <input
                                                    ...
                                                    value={item[`${day}AM`] || ''}
                                                    onChange={(e) =>
                                                        handleInputTotal(index, `${day}AM`, e.target.value)
                                                    }
                                            ...
                                            <td className="border p-1">
                                                <input
                                                    ...
                                                    value={item[`${day}PM`] || ''}
                                                    onChange={(e) =>
                                                        handleInputTotal(index, `${day}PM`, e.target.value)
                                                    }
                                                    className="border border-gray-300 px-3 py-1 text-sm rounded-md w-16 focus:ring-2 focus:ring-yellow-300 focus:outline-none"
                                                />
                                            </td>
    ...
    const handleSoldChange = (index, value) => {
        const updatedItems = [...items]
        const soldValue = parseFloat(value) || 0
        updatedItems[index].sold = soldValue
        const startingValue = updatedItems[index].starting || 0
        updatedItems[index].ending = startingValue - soldValue
        const priceValue = updatedItems[index].price || 0
        updatedItems[index].sales = priceValue * updatedItems[index].sold
        setItems(updatedItems)
    }
    ...
    return (
        ...
        >
            <div className="bg-white/80 backdrop-blur-lg p-6 w-full max-w-5xl mx-auto rounded-lg shadow-lg ">
                                ...
                                {['starting', 'delivery', 'waste', 'ending', 'sold'].map((field) => (
                                    <td key={field} className="p-2">
                                        <TableInput
                                            ...
                                            onChange={
                                                field === 'sold'
                                                    ? (e) => handleSoldChange(index, e.target.value)
                                                    : (e) => handleInputChange(index, field, e.target.value)
                                            }
                                        />
                                    </td>
                                ))}
                    ...
                    <tbody>
                        {cashBreakdown.map((item, index) => (
                            <tr key={index} className="hover:bg-gray-50">
                                ...
                                <td className="p-2">
                                    {item.denomination === 'Coins' ? item.quantity : item.denomination * item.quantity}
                                </td>
                            </tr>
                        ))}
                    </tbody>
                ...
                <div className="mt-4 text-lg font-semibold text-gray-800">Total Cash: P {calculateTotal()}</div>
                ...
                <h3 className="text-lg font-bold mt-8 text-center">
                    ITO AY PATUNAY NA LAHAT NG DETALYE AT PERA NA NAKASAAD SA REPORT NA ITO AY WASTO AT KUMPLETO
                </h3>
                <div className="grid gap-4 mt-4 md:grid-cols-2">
                    <div className="flex flex-col">

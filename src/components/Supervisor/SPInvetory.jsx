...
const inventoryData = [
  { item: "Buns", price: 5.00, nagcarlan: 0, rizal: 0, sanPablo: 0, rowAction: null },
  { item: "HD Buns", price: 5.00, nagcarlan: 0, rizal: 0, sanPablo: 0, rowAction: null },
  { item: "FL Buns", price: 12.00, nagcarlan: 0, rizal: 0, sanPablo: 0, rowAction: null },
  { item: "Patty", price: 15.00, nagcarlan: 0, rizal: 0, sanPablo: 0, rowAction: null },
  { item: "FL Dog", price: 44.00, nagcarlan: 0, rizal: 0, sanPablo: 0, rowAction: null },
  { item: "Hung. Chiz", price: 65.00, nagcarlan: 0, rizal: 0, sanPablo: 0, rowAction: null },
  { item: "Cheese", price: 5.00, nagcarlan: 0, rizal: 0, sanPablo: 0, rowAction: null },
  { item: "Chori Patty", price: 22.50, nagcarlan: 0, rizal: 0, sanPablo: 0, rowAction: null },
  { item: "Bacon", price: 25.00, nagcarlan: 0, rizal: 0, sanPablo: 0, rowAction: null },
  { item: "Cheesy HD", price: 13.50, nagcarlan: 0, rizal: 0, sanPablo: 0, rowAction: null },
  { item: "Egg", price: 15.00, nagcarlan: 0, rizal: 0, sanPablo: 0, rowAction: null },
  { item: "Mineral", price: 16.00, nagcarlan: 0, rizal: 0, sanPablo: 0, rowAction: null },
  { item: "Red Tea", price: 35.00, nagcarlan: 0, rizal: 0, sanPablo: 0, rowAction: null },
  { item: "M. Dew PET", price: 25.00, nagcarlan: 0, rizal: 0, sanPablo: 0, rowAction: null },
  { item: "Pepsi PET", price: 25.00, nagcarlan: 0, rizal: 0, sanPablo: 0, rowAction: null },
  { item: "7Up PET", price: 25.00, nagcarlan: 0, rizal: 0, sanPablo: 0, rowAction: null },
  { item: "Miranda PET", price: 25.00, nagcarlan: 0, rizal: 0, sanPablo: 0, rowAction: null },
  { item: "Rootbeer PET", price: 25.00, nagcarlan: 0, rizal: 0, sanPablo: 0, rowAction: null },
  { item: "Tropicana PET", price: 25.00, nagcarlan: 0, rizal: 0, sanPablo: 0, rowAction: null },
  { item: "Sting PET", price: 25.00, nagcarlan: 0, rizal: 0, sanPablo: 0, rowAction: null },
  { item: "Gatorade PET", price: 25.00, nagcarlan: 0, rizal: 0, sanPablo: 0, rowAction: null },
];
const SPInventory = () => {
    ...
    setInventory(prevInventory => {
      const updatedInventory = prevInventory.map((item, idx) => {
        if (idx === index) {
          ...
          return { ...item, [selectedLocation]: newQuantity, 
            rowAction: type === 'add' ? 'add' : 'sub',
          };
        }
  ...
  const handleTransfer = async (index) => {
    const transferQty = Number(quantityInput[index]) || 0; // Ensure the value is a number
    const transferTo = transferLocation[index];

    // Validation: Ensure a valid quantity and destination location
    ...
    try {
      // Update Firestore for the source location
      ...
      // Update Firestore for the destination location
      const destDocRef = doc(db, "inventory", `${sourceItem.item}_${transferTo}`);
      const destDocSnapshot = await getDoc(destDocRef);

      let newDestinationQuantity = transferQty;

      if (destDocSnapshot.exists()) {
        // If the destination item already exists, get its current quantity
        const currentDestQuantity = destDocSnapshot.data().quantity || 0;
        newDestinationQuantity = currentDestQuantity + transferQty;
      }

      // Ensure destination quantity is updated correctly
      ...
      setInventory((prevInventory) => {
        ...
        updatedInventory[index][transferTo] =
          (prevInventory[index][transferTo] || 0) + transferQty;
        return updatedInventory;
      });
  ...
  return (
    <div className="min-h-screen overflow-hidden bg-gray-100 p-6 flex items-center justify-center"
      ...
      <div className="bg-white/90 rounded-lg shadow-lg backdrop-blur-md p-4 sm:p-8 w-full max-w-lg md:max-w-3xl lg:max-w-5xl">
        ...
        <p className="text-black font-medium text-base sm:text-lg mb-4 sm:mb-6 text-center sm:text-right">Date: {currentDate}</p>

        {/* Location Selection */}
        <div className="flex flex-col sm:flex-row items-center sm:justify-start mb-4 sm:mb-6">
          <label htmlFor="location" className="text-gray-700 font-semibold mr-0 sm:mr-3 mb-2 sm:mb-0">Select Location:</label>
          <select
            id="location"
          ...
          <table className="min-w-full bg-white border border-gray-300 rounded-lg shadow-lg">
            <thead>
              <tr className="bg-yellow-300 text-gray-800">
                <th className="px-2 sm:px-4 py-3 border-b-2 border-yellow-400 text-left text-xs sm:text-sm">Items</th>
                <th className="px-2 sm:px-4 py-3 border-b-2 border-yellow-400 text-left text-xs sm:text-sm">Price</th>
                <th className="px-2 sm:px-4 py-3 border-b-2 border-yellow-400 text-left text-xs sm:text-sm">Quantity</th>
                <th className="px-2 sm:px-4 py-3 border-b-2 border-yellow-400 text-left text-xs sm:text-sm">Total Price</th>
                <th className="px-2 sm:px-4 py-3 border-b-2 border-yellow-400 text-left text-xs sm:text-sm">Actions</th>
              </tr>
            </thead>
            <tbody>
              {inventory.map((item, index) => (
                <tr
                  ...
                  `}
                >
                  <td className="px-2 sm:px-4 py-3 border-b border-gray-200 text-gray-700 text-xs sm:text-sm">{item.item}</td>
                  <td className="px-2 sm:px-4 py-3 border-b border-gray-200 text-gray-700 text-xs sm:text-sm">{item.price.toFixed(2)}</td>
                  <td className="px-2 sm:px-4 py-3 border-b border-gray-200 text-gray-700 text-xs sm:text-sm">{item[selectedLocation]}</td>
                  <td className="px-2 sm:px-4 py-3 border-b border-gray-200 text-gray-700 text-xs sm:text-sm">{calculateTotalPrice(item).toFixed(2)}</td>
